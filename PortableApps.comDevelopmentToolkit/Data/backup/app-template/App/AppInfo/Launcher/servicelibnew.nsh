...
;	processid  - get running service process id
;	config     - Changes the configuration parameters of a service
;              Parameters:
; 								_In_      SC_HANDLE hService,
; 				servicetype		_In_      DWORD     dwServiceType,
; 				starttype		_In_      DWORD     dwStartType,
;				errorcontrol	_In_      DWORD     dwErrorControl,
; 				path			_In_opt_  LPCTSTR   lpBinaryPathName,
;								_In_opt_  LPCTSTR   lpLoadOrderGroup,
; 								_Out_opt_ LPDWORD   lpdwTagId,
; 				depend			_In_opt_  LPCTSTR   lpDependencies,
; 				user			_In_opt_  LPCTSTR   lpServiceStartName,
; 				password		_In_opt_  LPCTSTR   lpPassword,
;				display			_In_opt_  LPCTSTR   lpDisplayName
;
; Usage:
;   Method 1:
;     Push "action"
...
!ifndef SERVICELIB
  ...
  !macro FUNC_SERVICE UN
	...
	StrCmp $3 "config" lbl_config			;add 2017/07/04
    Goto lbl_done
    ...
    lbl_config:
      ...
      Push $R7 ;display
      Push $R8 ;description
	  Push $R9 ;errorcontrol
 
      !insertmacro CALL_GETPARAM $R1 "depend" "n" "lbl_dependc"
        StrCpy $R1 't "$R1"'
      lbl_dependc:
      StrCmp $R1 "n" 0 lbl_machine ;old name of depend param
      !insertmacro CALL_GETPARAM $R1 "machine" "n" "lbl_machinec"
        StrCpy $R1 't "$R1"'
      lbl_machinec:
 
      !insertmacro CALL_GETPARAM $R2 "user" "n" "lbl_userc"
        StrCpy $R2 't "$R2"'
      lbl_userc:
 
      !insertmacro CALL_GETPARAM $R3 "password" "n" "lbl_passwordc"
        StrCpy $R3 't "$R3"'
      lbl_passwordc:
 
      !insertmacro CALL_GETPARAM $R4 "servicetype" "${SERVICE_NO_CHANGE}" "lbl_servicetypec"
      lbl_servicetypec:
 
      !insertmacro CALL_GETPARAM $R5 "starttype" "${SERVICE_NO_CHANGE}" "lbl_starttypec"
      lbl_starttypec:
 
      !insertmacro CALL_GETPARAM $R6 "path" "n" "lbl_pathc"
	  StrCpy $R6 't "$R6"'
      lbl_pathc:
 
      !insertmacro CALL_GETPARAM $R7 "display" "n" "lbl_displayc"
	  StrCpy $R7 't "$R7"'
      lbl_displayc:
 
      !insertmacro CALL_GETPARAM $R9 "errorcontrol" "${SERVICE_NO_CHANGE}" "lbl_errorcontrolc"
      lbl_errorcontrolc:
	  
      System::Call 'advapi32::ChangeServiceConfig${APITAG}(i r5, i R4, i R5, i R9, \
                                             $R6, n, n, $R1, $R2, $R3, $R7) i.r6 ?e' 
	  Pop $0
