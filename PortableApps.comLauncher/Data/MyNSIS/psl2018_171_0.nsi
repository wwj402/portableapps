...
!define LAUNCHERDIR "d:\ThinAppPortable\Captures\SDL_PassoloLauncher2018_18.x.x.x\%ProgramFilesDir%\SDL Passolo 2018"
...
!macroend
	...
	${EndIf}
	StrCpy $uilocalename "$1"
	; MessageBox MB_OK "$uilanguage, $1"

	${If} ${FileExists} "$homedir\${ENDIR}\${LANGUAGEFILES}"
	${AndIf} ${FileExists} "$homedir\${ENDIR}\${USERLANGUAGEFILES}"
		Call changelanguage_sfx
	${Else}
		Call changelanguage
	${EndIf}
	...
	${If} $0 == ""
		Exec "$homedir\${APPEXE}"
		; ExecDos::exec /ASYNC '"$homedir\${APPEXE}"' '' ''
		; ${Execute} "$homedir\${APPEXE}" "" $1
		; MessageBox MB_OK "$1"
	${Else}
		Exec '"$homedir\${APPEXE}" "$0"'
		; ExecDos::exec /ASYNC '"$homedir\${APPEXE}" "$0"' '' ''
...
Function "AppPatch"
	...
	${If} $R6 != ""
		...
		StrCpy $1 "$R9" "" "$0"
		; MessageBox MB_OK "$1"
		${If} ${FileExists} "${APPDIR}$1.bak"
			GetFileTime "${APPDIR}$1" $2 $3
			IntOp $2 $2 & $3
			Rename "${APPDIR}$1" "${APPDIR}$1_$2.bak"
		${Else}
...
Function "changelanguage"
	...
	Push $1
	Push $2
	StrCpy $0 "$EXEDIR\System_$uilocalename"
	${If} ${FileExists} "$0"
		md5dll::GetMD5File "$0\${IDFILEDIR}\${IDFILE}"
		Pop $1
		md5dll::GetMD5File "$EXEDIR\${IDFILEDIR}\${IDFILE}"
		Pop $2
		${If} $1 != $2
			${EnumProcessPaths} "PslFind" $PslFindOutput
			${If} PslFindPath != "${APPDIR}\${APPEXE}"
				CopyFiles /SILENT "$0\System\*.*" "$EXEDIR\System"
				CopyFiles /SILENT "$0\Macros\*.*" "$EXEDIR\Macros"
				CopyFiles /SILENT "$0\MacroTemplates\*.*" "$EXEDIR\MacroTemplates"
				SetShellVarContext all
				CopyFiles /SILENT "$0\Macros\*.*" "$DOCUMENTS\Passolo 2018\Macros"
				CopyFiles /SILENT "$0\MacroTemplates\*.*" "$DOCUMENTS\Passolo 2018\MacroTemplates"
		...
		${EndIf}
	${Else}
		md5dll::GetMD5File "$EXEDIR\${ENDIR}\${IDFILEDIR}\${IDFILE}"
		Pop $1
		md5dll::GetMD5File "$EXEDIR\${IDFILEDIR}\${IDFILE}"
		Pop $2
		${If} $1 != $2
			${EnumProcessPaths} "PslFind" $PslFindOutput
			${If} PslFindPath != "${APPDIR}\${APPEXE}"
				CopyFiles /SILENT "$EXEDIR\${ENDIR}\System\*.*" "$EXEDIR\System"
				CopyFiles /SILENT "$EXEDIR\${ENDIR}\Macros\*.*" "$EXEDIR\Macros"
				CopyFiles /SILENT "$EXEDIR\${ENDIR}\MacroTemplates\*.*" "$EXEDIR\MacroTemplates"
				SetShellVarContext all
				CopyFiles /SILENT "$EXEDIR\${ENDIR}\Macros\*.*" "$DOCUMENTS\Passolo 2018\Macros"
				CopyFiles /SILENT "$EXEDIR\${ENDIR}\MacroTemplates\*.*" "$DOCUMENTS\Passolo 2018\MacroTemplates"
				SetShellVarContext current
			${Else}
				Abort "$(Message2)"
			${EndIf}
	...
	StrCpy $0 1
	${Do}
		ReadINIStr $1 "$EXEDIR\$uilocalename\filters.dat" "StringFilter#$0" "Name"
		ReadINIStr $2 "${USERDIR}\filters.dat" "StringFilter#$0" "Name"
		${If} $1 != $2
			WriteINIStr "${USERDIR}\filters.dat" "StringFilter#$0" "Name" "$1"
		${EndIf}
		IntOp $0 $0 + 1
	${LoopUntil} ${Errors}
	ClearErrors
	StrCpy $0 1
	${Do}
		ReadINIStr $1 "$EXEDIR\$uilocalename\prjfilters.dat" "ProjectFilter#$0" "Name"
		ReadINIStr $2 "${USERDIR}\prjfilters.dat" "ProjectFilter#$0" "Name"
		${If} $1 != $2
			WriteINIStr "${USERDIR}\prjfilters.dat" "ProjectFilter#$0" "Name" "$1"
		${EndIf}
		IntOp $0 $0 + 1
	${LoopUntil} ${Errors}
	Pop $2
...
Function "changelanguage_sfx"
	${Select} $uilocalename
	${Case} "zh-CN"
		SetOverwrite on
		SetOutPath "${USERDIR}"
		File /r "${LAUNCHERDIR}\zh-CN\*.dat"
	${Case} "de-DE"
		SetOverwrite on
		SetOutPath "${USERDIR}"
