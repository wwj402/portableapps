...
; === Define constants ===
; **************************************************************************
!define EXEFULLDIR "d:\ThinAppPortable\Captures\PyCharm_2019.3.5_x64\%ProgramFilesDir(x64)%\JetBrains\PyCharm 2019.3.5\bin"
...
!define APPLICFILE		"$ProfileDir\config\pycharm.key"
...
Section "Main"
	...
	${Locate} "$EXEDIR" "/L=D /G=0 /M=PyCharm*" "gethomedir"
	IfFileExists "$AppBaseDir\*.*" SKIPOTHER 0
	ReadRegStr $0 HKLM "SOFTWARE\JetBrains\PyCharm\191.7141.48" ""
	IfFileExists "$0\*.*" 0 SKIPREG
	GetFullPathName $0 "$0\.."
	${Locate} "$0" "/L=D /G=0 /M=PyCharm*" "gethomedir"
	SKIPREG:
